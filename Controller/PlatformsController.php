...
class PlatformsController
{
    ...
    public function index()
    {
        ...
        $platforms = $this->model->get_platforms($sort, $filter, $this->limit, $offset);
        ...
        require __DIR__ . "/../View/tu/platformList.php";
    }
    public function load_platforms()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'load_platforms') {
            ...
            // Fetch data
            $platforms = $this->model->get_platforms($sort, $filter, $limit, $offset);
    ...
    public function get_platform_by_id($id)
{
    ...
    $platform = $this->model->get_platform_by_id($id, $limit, $offset);
    $total_games = $this->model->get_game_count_by_platform($id);

    include('./View/tu/platformDetail.php');
}
    ...
    public function load_more_games()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'load_more_games') {
        ...
        $platform_data = $this->model->get_platform_by_id($id, $limit, $offset);
        $games = $platform_data['games'] ?? [];
        $total = $this->model->get_game_count_by_platform($id);
