...
class TagsController
{
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'load_tags') {
    ...
    public function get_tag_by_id($id)
{
    ...
    $tags = $this->model->get_tag_by_id($id, $limit, $offset);
    $total_games = $this->model->get_game_count_by_tag($id);

    include('./View/tu/tagDetail.php');
}
    ...
    public function load_more_games()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'load_more_games') {
        ...
        $tag_data = $this->model->get_tag_by_id($id, $limit, $offset);
        $games = $tag_data['games'] ?? [];
        $total = $this->model->get_game_count_by_tag($id);
