...
<body>
    ...
    <form id="searchForm">
        ...
        <button type="button" onclick="loadDevelopers(1, false)">Search</button>
    </form>

    <div id="developerList">
        <?php foreach ($developers as $g): ?>
            <a class="developer-card" href="../../index.php?action=developer&id=<?= $g['id'] ?>">
                <img src="<?= '../../View/data/' . htmlspecialchars($g['background_image']) ?>" class="developer-img" alt="<?= htmlspecialchars($g['name']) ?>">
                <div class="developer-info">
                    <div class="developer-title"><?= htmlspecialchars($g['name']) ?></div>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        ...
                        if (!append) $('#developerList').empty();

                        developers.forEach(p => {
                            const html = `
                                <a class="developer-card" href="../../index.php?action=developer&id=${p.id}">
                                    <img src="../../View/data/${p.background_image}" class="developer-img" alt="${p.name}">
                                    <div class="developer-info">
                                        <div class="developer-title">${p.name}</div>
                        ...
                        if (page >= response.total_pages || developers.length === 0) $('#loadMoreBtn').hide();
        ...
        $('#title, #sort_by').on('input change', function () {
            currentPage = 1;
            loadDevelopers(currentPage, false);
        });

        $('#loadMoreBtn').on('click', function () {
            currentPage++;
            loadDevelopers(currentPage, true);
        });
